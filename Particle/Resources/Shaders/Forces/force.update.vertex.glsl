...
float Noise(int o, double p, vec3 P);
float cnoise(vec3 P);
float snoise(vec3 v);
...
void main()
{
    ...
    // float lat = Noise(
    //     octaves, persistence, 
    //     vec3(0.0, yoffset + index.y, index.z * 0.01)
    // ) * TWO_PI * 4;
    float lat = snoise(
        vec3(0.0, yoffset + index.y, index.z * 0.01)
    ) * TWO_PI * 4;
    ...
    // float noise = Noise(
    //     octaves, persistence, 
    //     vec3(index.x * 0.01, yoffset + index.y, index.z * 0.01)
    // );
    float noise = snoise(
        vec3(index.x * 0.01, yoffset + index.y, index.z * 0.01)
    );
    float lon = noise * TWO_PI * 4;
    ...
    outForce = force * maxMagnitude / length(force);
    // outForce = normalize(force) * (maxMagnitude);
}
